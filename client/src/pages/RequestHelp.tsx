...
export default function RequestHelp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({
        type: 'error',
        message: error.response?.data?.error || t('requestHelp.errors.submitFailed'),
      });
    } finally {
      setLoading(false);
    }
        ...
        <div className="glass-strong border border-white/10 rounded-2xl p-8 md:p-12 shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-2 ml-1 uppercase tracking-wider">{t('requestHelp.fields.email')}</label>
                <input
                  type="email"
                  name="email"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-2 ml-1 uppercase tracking-wider">{t('requestHelp.fields.phone')}</label>
                <input
                  type="tel"
                  name="phone"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                >
                  <option value="">{t('requestHelp.placeholders.serviceType')}</option>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-2 ml-1 uppercase tracking-wider">{t('requestHelp.fields.priority')}</label>
                <select
                  name="priority"
                  ...
                  className="w-full appearance-none bg-slate-950/50 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all cursor-pointer"
                >
                  <option value="low">{t('requestHelp.priority.low')}</option>
                  <option value="medium">{t('requestHelp.priority.medium')}</option>
                  <option value="high">{t('requestHelp.priority.high')}</option>
                  <option value="urgent">{t('requestHelp.priority.urgent')}</option>
                </select>
              </div>
            </div>

            <div>
              <label className="block text-xs font-medium text-slate-400 mb-2 ml-1 uppercase tracking-wider">{t('requestHelp.fields.subject')}</label>
              <input
                type="text"
                name="subject"
                ...
                onChange={handleChange}
                required
              placeholder={t('requestHelp.placeholders.subject')}
                className="w-full bg-slate-950/50 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all placeholder:text-slate-600"
              />
            </div>

            <div>
              <label className="block text-xs font-medium text-slate-400 mb-2 ml-1 uppercase tracking-wider">{t('requestHelp.fields.description')}</label>
              <textarea
                name="description"
                ...
                required
                rows={6}
              placeholder={t('requestHelp.placeholders.description')}
