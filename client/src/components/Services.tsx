...
function Services() {
  ...
  return (
    <section ref={sectionRef} className="py-16 sm:py-24 md:py-32 relative z-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-10 sm:mb-12 md:mb-16 gap-4 sm:gap-6">
          <div className="max-w-2xl">
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-medium text-white tracking-tight mb-3 sm:mb-4 md:mb-6 [text-wrap:balance]">{t('homeServices.title')}</h2>
            <p className="text-slate-400 text-sm sm:text-base md:text-lg font-light leading-relaxed">{t('homeServices.subtitle')}</p>
          </div>
          <LangLink to="/services#service-details" className="text-white hover:text-indigo-300 text-sm font-medium inline-flex items-center gap-2 transition-colors border-b border-transparent hover:border-indigo-300 pb-0.5 shrink-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={index}
                className={`group card-glow p-5 sm:p-6 md:p-8 rounded-2xl glass border ${colors.border} ${colors.hoverBorder} hover:bg-white/[0.05] transition-all duration-500 backdrop-blur-md relative overflow-hidden ${animationClass} ${delayClass}`}
              ...
              >
                ...
                <div className={`h-11 w-11 sm:h-12 sm:w-12 rounded-lg ${colors.bg} border ${colors.border} flex items-center justify-center ${colors.text} mb-4 sm:mb-6 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 relative z-10 shadow-lg`}>
                  <Icon name={service.icon} width={22} />
                </div>
                <h3 className="text-lg sm:text-xl font-semibold text-white mb-2 sm:mb-3 relative z-10 group-hover:text-indigo-300 transition-colors">{t(service.title)}</h3>
